...
class PlugAPI extends EventEmitter3 {
    constructor(authenticationData, callback) {
        ...
        store(this)._messageHandler = (msg) => {
            ...
            switch (type) {
                ...
                case PlugAPI.events.GRAB:
                    store(this)._room.setGrab(data.raw);
                    break;
                ...
                case PlugAPI.events.MODERATE_STAFF:
                    if (Array.isArray(data.raw.u) && data.raw.u.length > 0) {
                ...
                case PlugAPI.events.API_FLOOD:
                case PlugAPI.events.IP_BANNED:
        ...
        /**
         * Should the bot be able to delete all chat regardless of role?
         * Note: This will NOT delete gRole chat.
         * @type {Boolean}
         */
    ...
    close(reconnecting) {
        ...
        if (store(this)._ws != null && Object.is(store(this)._ws.connection.readyState, WebSocket.OPEN)) {
            ...
            store(this)._ws.on('error', () => {});
            store(this)._ws.close(true);
        }
        if (!(Object.is(typeof reconnecting, 'boolean') && reconnecting)) {
            store(this)._ws.removeAllListeners();
        }
        store(this)._room = new Room();
    }

    /**
     * Get a history of chat messages. (Limit 512 messages)
    ...
    /**
     * Get a history of songs played. (Limit 50 songs)
    ...
    /**
     * Get historyID of the currently playing media. This is used for voting, grabbing, skipping, and various other purposes
     * @returns {String} A string representing the historyID of the currently playing media
     *
     * @example
     * bot.getHistoryID();
     */
     ...
     * @param {Boolean} append Whether to add song to the bottom or top. (true by default will put it at the bottom.)
     * @param {RESTCallback} [callback] Callback function
     * @returns {Boolean} If the REST request got queued
     *
     * @example
     ...
     */
    addSongToPlaylist(pid, sdata, append, callback) {
        if (!Object.is(typeof append, 'boolean')) {
            callback = append;
            append = true;
        }
    ...
    /**
     * Change the short description of the community
     * @param {String} description New community short description
     * @param {RESTCallback} [callback] Callback function
     * @returns {Boolean} If the REST request got queued
     *
     * @example
    ...
    /**
     * Change the long description of the community
     * @param {String} description New community long description
     * @param {RESTCallback} [callback] Callback function
     * @returns {Boolean} If the REST request got queued
     *
     * @example
     * bot.changeRoomLongDescription('My room is better than yours. #420');
     */
    changeRoomLongDescription(description, callback) {
        if (!store(this)._room.getRoomMeta().slug || !this.havePermission(undefined, PlugAPI.ROOM_ROLE.COHOST) || Object.is(store(this)._room.getRoomMeta().long_description, description) || this.guest) {
            return false;
        }
     ...
     * @param {Boolean} shouldCycle True if DJ Cycle should be enabled, false if not.
     * @param {RESTCallback} [callback] Callback function
     * @returns {Boolean} If the REST request got queued
     *
     * @example
    ...
    changeDJCycle(shouldCycle, callback) {
        if (!store(this)._room.getRoomMeta().slug || !this.havePermission(undefined, PlugAPI.ROOM_ROLE.MANAGER) || Object.is(store(this)._room.getBoothMeta().shouldCycle, shouldCycle) || this.guest) {
            return false;
        }
    ...
    /**
     * Set your badge. Be sure you only use badges that are available {@link PlugAPI#getBadges}
     * @param {String} id The badgeID to set.
     * @param {RESTCallback} [callback] Callback function
     * @returns {Boolean} If the REST request got queued
     * @since 5.1.0
     *
    ...
    setBadge(id, callback) {
        if (!store(this)._room.getRoomMeta().slug || !Object.is(typeof id, 'string') || this.guest) return false;
        store(this)._queueREST('PUT', endpoints.USER_SET_BADGE, {
            id
        }, callback);
     ...
     * @param {Function} [callback] optional callback for async.
     * @returns {User.badge} badgeID of the current badge
     * @since 5.1.0
     *
     * @example
     * bot.getBadge();
     */
    getBadge(callback) {
        if (Object.is(this.getSelf(), null)) return null;

        return Object.is(typeof callback, 'function') ? callback(this.getSelf().badge) : this.getSelf().badge;
    }
    ...
    getBadges(callback) {
        if (!store(this)._room.getRoomMeta().slug || this.guest) return false;
        store(this)._queueREST('GET', endpoints.USER_GET_BADGES, undefined, callback);
     ...
     * @prop {Number} GLOBAL_ROLES.PROMOTER. Global role of 500. Promoters.
     * @prop {Number} GLOBAL_ROLES.PLOT Global role of 750. PLOT members.
     * @prop {Number} GLOBAL_ROLES.VOLUNTEER Global role of 2000. Currently none are set as this.
     * @prop {Number} GLOBAL_ROLES.MODERATOR Global role of 25000. Site Moderators.
...
// Set static instance members to be prototype members
Object.entries(Object.getOwnPropertyDescriptors(PlugAPI)).filter(([key, descriptor]) => Object.is(typeof descriptor.get, 'function')).map(([key]) => key).forEach((item) => {
    PlugAPI.prototype[item] = PlugAPI[item];
});

// Deprecated functions. TODO: remove in v6
PlugAPI.prototype.createLogger = util.deprecate(() => {}, 'PlugAPI.createLogger is now a noop function and will be removed in V6. Use PlugAPI.logger instead and refer to https://jethrologger.github.io/documentation/jethro-node/ for documentation.');
PlugAPI.prototype.setLogger = util.deprecate(() => {}, 'PlugAPI.setLogger is now a noop function and will be removed in V6');
PlugAPI.prototype.getLoggerSettings = util.deprecate(() => {}, 'PlugAPI.getLoggerSettings is now a noop function and will be removed in V6. Use PlugAPI.logger instead and refer to https://jethrologger.github.io/documentation/jethro-node/ for documentation.');
 ...
 * @prop {String} data.type The message type (mention, emote, message)
 *
 * @example
