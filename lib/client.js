        ...
        request({
            ...
            if (!err) {
                if (data && data.status !== 'ok' || res && res.statusCode !== 200 || err) {
                    handleErrors.error('Login Error: ' + (data && data.status !== 'ok' && 'API Status: ' + data.status || '') + ' ' + (res && res.statusCode !== 200 && 'HTTP Status: ' + res.statusCode + ' - ' + http.STATUS_CODES[res.statusCode] || '') + ' ' + (err && err || ''), callback);
                } else {
                    _cookies.fromHeaders(res.headers);
                    loggingIn = false;
                    if (typeof callback === 'function') {
