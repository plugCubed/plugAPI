...
/**
 * Should everything be able to delete all chat?
 * @type {Boolean}
 */
const deleteAllChat = false;
...
function sendREST(opts, callback) {
    request(opts, (err, res, body) => {
        if (err || res && res.statusCode !== 200) {
            const error = `[REST Error]' ${err} HTTP Status:  ${res.statusCode} ${http.STATUS_CODES[res.statusCode]}`;
...
function receivedChatMessage(messageData) {
    ...
    if (!disconnectedUser && messageData.message.startsWith(commandPrefix) && (that.processOwnMessages || messageData.from.id !== room.getSelf().id)) {
        ...
        messageData.respond = function() {
            ...
            return that.sendChat(`@${this.from.username} ${message.join(' ')}`);
        };
    ...
    if (room.getUser() !== null && messageData.message.includes(`@${room.getUser().username}`)) {
...
function performLoginCredentials(callback) {
    ...
    request.get({
        ...
        if (res && res.statusCode !== 200 || err) {
            handleErrors.error(`Login Error: HTTP Status: ${res.statusCode}- ${http.STATUS_CODES[res.statusCode]} Error: ${err}`, callback);
        }
        try {
        ...
        request({
            ...
            if (!error) {
                if (data && data.status !== 'ok' || res && res.statusCode !== 200 || err) {
                    handleErrors.error(`Login Error: API Status: ${data.status}) HTTP Status: ${result.statusCode}- ${http.STATUS_CODES[result.statusCode]}`, callback);
                } else {
                    _cookies.fromHeaders(result.headers);
                    loggingIn = false;
                    if (typeof callback === 'function') {
...
PlugAPI.prototype.moderateDeleteChat = function(cid, callback) {
    ...
    if (!deleteAllChat || (user !== null ? room.getPermissions(user).canModChat : this.havePermission(undefined, PlugAPI.ROOM_ROLE.BOUNCER))) {
