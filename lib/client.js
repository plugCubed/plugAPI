...
function sendREST(opts, callback) {
    request(opts, function(err, res, body) {
        if (err || res.statusCode !== 200) {
            logger.error('[REST Error]', (err == null ? ' ' : err), res.statusCode);
            callback((err != null ? err : (res.statusCode !== 200 ? res.statusCode : null)), null);
            return;
        }
        try {
...
function messageHandler(msg) {
    ...
    switch (type) {
        case 'ack':
            ...
            queueREST('GET', endpoints.USER_INFO_ME, null, function(err, data) {
        ...
        case PlugAPI.events.ADVANCE:
            if (that.fixNullUsers && room.getUser(data.currentDJ) == null) {
                queueREST('GET', endpoints.USER_INFO + data.currentDJ, undefined, function(err, data) {
                    if (!err) room.addUser(data[0]);
                });
            }
        ...
        case PlugAPI.events.CHAT:
            if (that.fixNullUsers && data.from == null) {
                queueREST('GET', endpoints.USER_INFO + data.raw.uid, undefined, function(err, data) {
                    if (!err) room.addUser(data[0]);
                });
            }
        ...
        case PlugAPI.events.DJ_LIST_UPDATE:
            if (that.fixNullUsers) {
                for (var i = 0; i < 3; i++) {
                    if (room.getUser(data[i]) != null) continue;
                    queueREST('GET', endpoints.USER_INFO + data[i], undefined, function(err, data) {
                        if (!err) room.addUser(data[0]);
                    });
                }
            }
...
function PerformLoginCredentials(callback) {
    ...
    request.get({
        ...
        if (res.statusCode !== 200 || err) {
            handleErrors.error('Login Error: ' + (res.statusCode !== 200 && ' HTTP Status: ' + res.statusCode) + ' ' + (err && 'Error : ' + err || ' '), callback);
        }
        try {
        ...
        request({
            ...
            if (!err) {
                if (data.status !== 'ok' || res.statusCode !== 200 || err) {
                    handleErrors.error('Login Error: ' + (data.status !== 'ok' && 'API Status: ' + data.status) + ' ' + (res.statusCode !== 200 && 'HTTP Status: ' + res.statusCode) + ' ' + (err && 'Error: ' + err || ' '), callback);
                } else {
                    _cookies.fromHeaders(res.headers);
                    loggingIn = false;
                    if (typeof callback === 'function') {
...
var PlugAPI = function(authenticationData, callback) {
    ...
    /**
     * Should null users be automatically fetched and added to the room?
     * @type {Boolean}
     */
    this.fixNullUsers = false;
...
/**
 * Get external user object and update if they're in the community
 * @param {Number} [uid]
 * @param {RESTCallback} callback
 * @returns {Boolean} If the REST request got queued
 */
PlugAPI.prototype.getExternalUser = function(uid, callback) {
    if (typeof uid !== 'number') return false;

    queueREST('GET', endpoints.USER_INFO + uid, undefined, function(err, data) {
        if (err) {
            if (typeof callback === 'function') callback(err, null);
            return;
        }

        //Update internal user, if they're in the community
        if (room.getUser(uid) != null) {
            data[0].i = data[0].id;
            delete data[0].id;

            room.updateUser(data[0]);

            data[0].id = data[0].i;
            delete data[0].i;
        }

        if (typeof callback === 'function') callback(null, data[0]);
    });
