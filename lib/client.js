...
class PlugAPI extends EventEmitter3 {
    constructor(authenticationData, callback) {
        ...
        /**
         * Slug of this.room, that the bot is currently connecting to
         * @type {null|String}
         * @private
         */
    ...
    joinRoom(roomSlug, callback) {
        this.queueREST('POST', 'rooms/join', {
            ...
            if (err) {
                if (isFinite(err)) {
                    if (err >= 400 && err < 500) {
                        handleErrors.error(`Invalid this.room URL. Please Check If the this.roomSlug ${roomSlug} is correct`, callback);
                    } else {
    ...
    connect(roomSlug) {
        if (!roomSlug || typeof roomSlug !== 'string' || roomSlug.length === 0 || roomSlug.indexOf('/') > -1) {
            throw new Error('Invalid this.room name');
        }

        if (this.connectingRoomSlug != null) {
            logger.error('PlugAPI', 'Already connecting to a this.room');
     ...
     * @returns {Array} An array of all admins in this.room
     */
    getAdmins() {
     ...
     * @returns {Array} An array of all ambassadors in this.room
     */
    getAmbassadors() {
     ...
     * @returns {User|null} The Host as a user object if they are in this.room, otherwise null
     */
    getHost() {
     ...
     * @returns {User[]} A User array of all staff in this.room
     */
    getStaff() {
     ...
     * @returns {User[]} An array of all users in this.room
     */
    getUsers() {
    ...
    activatePlaylist(pid, callback) {
        ...
        if (!this.room.getRoomMeta().slug) {
            if (typeof callback === 'function') {
                return callback(new Error('Not in a this.room'), null);
            }
     ...
     * @param {string} roomSlug for guests to get auth code from
     * @param {Function} callback Callback function
     * @private
     */
    getAuthCode(roomSlug, callback) {
        ...
        if (!roomSlug) {
            ...
            const url = `${this.baseUrl}/${typeof roomSlug === 'string' ? roomSlug : ''}`;
            ...
            got(url, {
                ...
                if (body && body.body && body.body.indexOf('_jm') > -1 && body.body.indexOf('_st') > -1) {
                    this._authCode = body.body.split('_jm')[1].split('"')[1];
    ...
    /**
     * Initialize the this.room with this.roomstate data
     * @param {Object} data this.roomstate Data
     * @param {Function} callback Callback function
     * @private
     */
    initRoom(data, callback) {
        ...
        this.queueREST('GET', endpoints.HISTORY, undefined, this.room.setHistory);
     ...
     * @example
     *
     * bot.on(PlugAPI.events.BAN, (data) => { console.log(`I Was banned for ${data.d === 'h' ? 'an hour' : data.d === 'f' ? 'permanently' : data.d === 'd' ? 'day'} because of Reason #${data.r}`) })
     *
