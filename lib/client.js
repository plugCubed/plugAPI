...
function queueREST(method, endpoint, data, callback, skipQueue) {
    ...
    const opts = {
        ...
        headers: Object.assign(headers, {
            cookie: that.jar.getCookieStringSync('https://plug.dj'),
...
function sendREST(opts, callback) {
    ...
    .catch(function(err) {
        logger.error('REST Error', `Route: ${opts.url} ${(err ? err : '')} Guest Mode: ${that.guest}`);

        return callback(`Route: ${opts.url} ${(err ? err : '')} Guest Mode: ${that.guest}`, null);
    });
}
...
function connectSocket(roomSlug) {
    ...
    ws.on('open', function() {
        logger.success('plug.dj Socket Server', chalk.green(`Connected as a ${that.guest ? 'guest' : 'user'}`));
...
function performLoginCredentials(callback) {
    ...
    if (that.guest) {
        ...
        }).catch(function(guestError) {
            return callback(guestError, null);
        });
    } else {
        got(baseUrl, {
            headers
        }).then(function(indexBody) {
            const csrfToken = /csrf\s?=\s?"(.*?)"/.exec(indexBody.body);
...
PlugAPI.prototype.connect = function(roomSlug) {
    ...
    if (!that.jar.getCookieStringSync('https://plug.dj')) {
        setImmediate(function() {
 ...
 */
PlugAPI.prototype.getBaseURL = function() {
    return baseUrl;
};
 ...
 * @returns {Boolean} True if set. False if not
 */
PlugAPI.prototype.setBaseURL = function(url) {
 ...
 */
PlugAPI.prototype.getSocketURL = function() {
    return socketUrl;
};
 ...
 * @returns {Boolean} True if set. False if not
 */
PlugAPI.prototype.setSocketURL = function(url) {
