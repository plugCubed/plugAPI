...
function sendREST(opts, callback) {
    request(opts, function(err, res, body) {
        if (err || (res && res.statusCode !== 200) || !res) {
            logger.error('REST Error', `Route: ${opts.url} ${(err ? err : '')}${res && res.statusCode ? `HTTP Status: ${res.statusCode} - ${http.STATUS_CODES[res.statusCode]}` : ''} Guest Mode: ${that.guest}`);

            return callback(`Route: ${opts.url} ${(err ? err : '')}${res && res.statusCode ? `HTTP Status: ${res.statusCode} - ${http.STATUS_CODES[res.statusCode]}` : ''} Guest Mode: ${that.guest}`, null);
        }
...
function receivedChatMessage(messageData) {
    ...
    if (messageData.message.indexOf(commandPrefix) === 0 && (that.processOwnMessages || messageData.from.id !== room.getSelf().id)) {
...
function getAuthCode(roomSlug, callback) {
    const url = typeof roomSlug === 'string' ? roomSlug : '/';

    request(url, function(err, res, body) {
        if (err) {
            logger.error('PlugAPI', 'Error getting auth code:', err);
        } else if (res && res.statusCode === 200 && body.indexOf('_jm') > -1 && body.indexOf('_st') > -1) {
            ...
            logger.error('PlugAPI', res && res.statusCode !== 200 ? `Error getting auth code: HTTP ${res.statusCode} - ${http.STATUS_CODES[res.statusCode]}` : 'Plug may be in maintenance or changed something.');
...
function performLoginCredentials(callback) {
    ...
    if (that.guest) {
        ...
        request.get({
            ...
            const csrfToken = /csrf\s?=\s?"(.*?)"/.exec(body);
...
PlugAPI.prototype.connect = function(roomSlug) {
    ...
    if (!jar.getCookieString('https://plug.dj')) {
        setImmediate(function() {
 ...
 * @returns {Boolean} If the REST request got queued
 */
PlugAPI.prototype.moderateLockWaitList = PlugAPI.prototype.moderateLockBooth = function(locked, clear, callback) {
