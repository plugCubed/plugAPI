...
//var isFinite = require('is-finite'); //isFinite is implemented in JS. No need to use another lib.
...
function receivedChatMessage(messageData) {
    ...
    if (!disconnectedUser && messageData.message.indexOf(commandPrefix) === 0 && (that.processOwnMessages || messageData.from.id !== room.getSelf().id)) {
        ...
        messageData.isFrom = function(ids, success, failure) {
            ...
            var isFrom = ids.indexOf(this.from.id) > -1;
...
function connectSocket(roomSlug) {
    ...
    ws = new websock('wss://godj.plug.dj:443/socket', {
    ...
    ws.on('error', function(a) {
        ...
        process.nextTick(reconnect(room, roomSlug));
    });
...
function sendWebsocket(data, options, cb) {
    ...
    if (ws !== null && ws.readyState === websock.OPEN) {
...
PlugAPI.prototype.addSongToPlaylist = function(pid, sdata, callback) {
    ...
    for (var i in sdata) {
        ...
        if (sdata[i].id === null) // On plug.dj, this ID is always 0, so it is not important apparently
            sdata[i].id = 0;
