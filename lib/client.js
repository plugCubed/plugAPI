...
class PlugAPI extends EventEmitter3 {
    constructor(authenticationData, callback) {
        ...
        store(this)._performLoginCredentials = (loginCallback) => {
            ...
            if (this.guest) {
                ...
                got(`${this.baseUrl}/_/mobile/init`, {
                    ...
                    if (Object.is(info.status, 'maintenanceMode')) throw new Error('Maintenance Mode');
        ...
        */
        ...
        if (Object.is(typeof authenticationData, 'object') || Object.is(authenticationData, null)) {
            this.guest = true;
    ...
    moderateSetRole(userID, roleID, callback) {
        ...
        if (!Object.is(user, null) ? store(this)._room.getPermissions(user).canModStaff : this.havePermission(undefined, PlugAPI.ROOM_ROLE.MANAGER)) {
            if (user && Object.is(user.role, roleID)) return false;
            if (Object.is(roleID, 0)) {
     ...
     * @prop {Number} ROOM_ROLE.RESIDENTDJ Room role of 1. User is a resident DJ.
     * @prop {Number} ROOM_ROLE.BOUNCER Room role of 2. User is a bouncer.
     * @prop {Number} ROOM_ROLE.MANAGER Room role of 3. User is a manager.
     * @prop {Number} ROOM_ROLE.COHOST Room role of 4. User is a cohost.
     * @prop {Number} ROOM_ROLE.HOST Room role of 5. User is a host.
     * @static
     */
     ...
     * @prop {Number} GLOBAL_ROLES.VOLUNTEER Global role of 2. Currently none are set as this.
     * @prop {Number} GLOBAL_ROLES.AMBASSADOR Global role of 3. Brand Ambassador role.
     * @prop {Number} GLOBAL_ROLES.LEADER Global role of 4. Currently none are set as this.
     * @prop {Number} GLOBAL_ROLES.ADMIN Global role of 5. Admin global role.
     * @static
     */
