...
class PlugAPI extends EventEmitter3 {
        ...
        store(this)._performLoginCredentials = (loginCallback) => {
            ...
            if (this.guest) {
                ...
                got(`${this.baseUrl}/_/mobile/init`, {
                 ...
                 .catch((err) => {
                     setImmediate(() => {
                         handleErrors.error(`Login Error: ${err ? err : ''}`, loginCallback);
                     });
                 });
            }
        ...
        store(this)._receivedChatMessage = (messageData) => {
            ...
            if (messageData.message.indexOf(this.commandPrefix) === 0 && (this.processOwnMessages || messageData.from.id !== store(this)._room.getSelf().id)) {
                ...
                if (!mutedUser) {
                    ...
                    console.log(this.deleteMessageBlocks, store(this)._lastMessageType, messageData.type, store(this)._lastMessageUID, messageData.raw.uid);
                    if (this.deleteCommands && (!this.deleteMessageBlocks && (store(this)._lastMessageType !== messageData.type || store(this)._lastMessageUID !== messageData.raw.uid))) {
        ...
        store(this)._messageHandler = (msg) => {
            ...
            switch (type) {
                ...
                case PlugAPI.events.VOTE:
                    store(this)._room.setVote(data.i, data.v);
                    break;
    ...
    setAvatar(avatar, callback) {
        if (!store(this)._room.getRoomMeta().slug || !avatar || this.guest) return false;
    ...
    playlistMoveMedia(pid, mid, beforeMid, callback) {
        if (!store(this)._room.getRoomMeta().slug || !pid || !mid || (Array.isArray(mid) && mid.length === 0) || !beforeMid || this.guest) return false;
    ...
    moderateMoveDJ(uid, index, callback) {
        if (!store(this)._room.getRoomMeta().slug || !this.havePermission(undefined, PlugAPI.ROOM_ROLE.MANAGER) || !store(this)._room.isInWaitList(uid) || !isFinite(index) || this.guest) {
            return false;
        }
    ...
    moderateSetRole(uid, role, callback) {
        if (!store(this)._room.getRoomMeta().slug || !isFinite(role) || role < 0 || role > 5 || this.guest) {
            return false;
        }
        ...
        if (user !== null ? store(this)._room.getPermissions(user).canModStaff : this.havePermission(undefined, PlugAPI.ROOM_ROLE.MANAGER)) {
            if (role === 0) {
                store(this)._queueREST('DELETE', endpoints.MODERATE_STAFF + uid, undefined, callback);
            } else {
