...
function PerformLoginCredentials(callback) {
    ...
    request.get({
        ...
        if (res && res.statusCode !== 200 || err) {
            throw new Error('LOGIN ERROR: Can\'t connect to plug.dj.' + (res && res.statusCode ? res.statusCode : ' ') + err && err);
        }
        try {
            _cookies.fromHeaders(res.headers);
            csrfToken = body.split('_csrf')[1].split('"')[1];
        } catch (e) {
            throw new Error('LOGIN ERROR: Can\'t get CSRF Token');
        }
        request({
            method: 'POST',
            uri: '/_/auth/login',
            ...
            if (data && data.status !== 'ok' || result && result.statusCode !== 200 || error) {
                throw new Error('LOGIN ERROR: ' + (res && res.statusCode ? ' Status: ' + res.statusCode : ' ') + error && error + (data && data.status ? ' Status: ' + data.status : ' '));
            } else {
...
var PlugAPI = function(authenticationData, callback) {
    if (!authenticationData) {
        throw new TypeError('You must pass the authentication data into the PlugAPI object to connect correctly');
    } else {
        if (!authenticationData.email) {
            throw new TypeError('Missing login e-mail');
        }
        if (!authenticationData.password) {
            throw new TypeError('Missing login password');
        }
    }
