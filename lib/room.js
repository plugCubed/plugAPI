 ...
 * @param {String} data.avatarID The user's avatar ID.
 * @param {String} [data.badge=null] The user's badge ID.
 * @param {Number} [data.gRole=0] The user's global role (0 if not a BA / Admin)
 * @param {Boolean} data.grab If the user has grabbed a song
 * @param {Number} [data.id=-1] The user's ID. Defaults to -1.
 ...
 * @param {String} [data.language=null] The user's language. Note that null is the same as en.
 ...
 * @param {Number} data.pp The user's Plug Points. Only shown for the bot user.
 * @param {Boolean} data.pw Whether the bot user is signed in via password or not.
 ...
 * @param {String} data.slug The user's slug to be used in a profile. https://plug.dj/@/slug (only exists for level > 5)
 ...
 * @param {Number} data.xp The user's XP. Only shown for the bot user.
...
class User {
    constructor(data, instance) {
        ...
        this.grab = instance.grabs[data.id] === 1;
        ...
        this.vote = instance.votes[data.id] != null ? instance.votes[data.id] === -1 ? -1 : 1 : 0;
...
class Room {
    constructor() {
        ...
        */
         ...
         * @type {number}
         */
        this.role = 0; // eslint-disable-line no-unused-vars
    ...
    isDJ(uid) {
        ...
        return this.booth.currentDJ === uid;
    }
    ...
    addUser(user) {
        ...
        // Don't add yourself
        if (user.id === this.mySelf.id) return;
    ...
    */
     ...
     */
    getUser(uid) {
        if (!uid || uid === this.mySelf.id) return this.getSelf();
        if (typeof uid === 'string') {
            uid = parseInt(uid, 10);
        }
        for (const i of this.users) {
            if (i.id === uid) return new User(i, this);
        }
    ...
    getRoomScore() {
        ...
        let i;
        const votesData = Object.keys(this.votes).length;
        const grabsData = Object.keys(this.votes).length;
        let votesLength = votesData.length;
        let grabs = grabsData.length;
