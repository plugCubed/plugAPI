 ...
 * @param {?String} data.badge The user's badge ID.
 ...
 * @param {?String} data.language The user's language. Note that null is the same as en.
 ...
 * @param {?status} [data.status=1] The user's status.
...
setInterval(() => {
    for (const i in this.mutes) {
        if (!this.mutes.hasOwnProperty(i)) continue;
        if (this.mutes[i] > 0) {
            this.mutes[i]--;
        }
    }
