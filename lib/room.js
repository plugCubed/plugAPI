 ...
 * @type {number}
 */
let role = 0; //eslint-disable-line
...
setInterval(() => {
    const reference = Object.keys(mutes);
...
Room.prototype.getRoomScore = function() {
    ...
    const reference = Object.keys(votes);
    ...
    const reference1 = Object.keys(grabs);

    for (const j of reference1) {
        if (j > 0) {
            result.grabs++;
        }
    }
