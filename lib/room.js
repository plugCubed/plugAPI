...
class Room {
    constructor() {
        ...
        /**
         * @private
         * @type {{shortDescription: string, longDescription: string, favorite: boolean, guests: number, hostID: number, hostName: string, id: number, minChatLevel: number, name: string, population: number, slug: string, welcome: string}}
         */
        this.meta = {
            shortDescription: '',
            longDescription: '',
    ...
    getPermissions(other) {
        ...
        permissions.canModBan = other.gRole < constants.GLOBAL_ROLES.VOLUNTEER && ((me.gRole > constants.GLOBAL_ROLES.VOLUNTEER) || me.role > other.role);
        permissions.canModChat = isAdmin || ((other.gRole < constants.GLOBAL_ROLES.MODERATOR) && ((me.role > constants.ROOM_ROLE.RESIDENTDJ) || me.gRole > constants.GLOBAL_ROLES.PLOT));
        permissions.canModMute = other.role < constants.ROOM_ROLE.RESIDENTDJ && other.gRole < constants.GLOBAL_ROLES.VOLUNTEER;
        permissions.canModStaff = isAdmin || (!Object.is(other.gRole, constants.GLOBAL_ROLES.ADMIN) && (!(me.gRole < constants.GLOBAL_ROLES.MODERATOR && me.role < constants.ROOM_ROLE.BOUNCER) && Math.max(me.role, me.gRole) > Math.max(other.role, other.gRole)));
    ...
    setRoomDescription(desc) {
        this.meta.shortDescription = desc;
    }

    setRoomLongDescription(desc) {
        this.meta.longDescription = desc;
    }
